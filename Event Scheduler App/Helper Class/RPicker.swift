...
class RPickerController: UIViewController {
    ...
    private let barViewheightForIpad: CGFloat = 68
    private let pickerHeight: CGFloat = 150
    private let pickerHeightForiPad: CGFloat = 300
    private let buttonWidth: CGFloat = 84
    private let buttonWidthForIpad: CGFloat = 168
    private let lineHeight: CGFloat = 0.5
    private let lineHeightforIpad: CGFloat = 1.0
    ...
    private func initialSetup() {
            ...
            if pickerType == .date {
                ...
                if #available(iOS 14.0, *) {
                    if datePickerStyle == .Wheel {
                            height = height + pickerHeightForiPad
        ...
        stackView.pinConstraints(view, left: 0, right: 0, bottom: -(20), height: height)
    ...
    private func setUpThemeMode(isDark: Bool) {
        ...
        if isDark {
            ...
            toolBarView.backgroundColor = UIColor.black//UIColor(hexString: "33A5FF")
        }
    }
    ...
    private lazy var toolBarView: UIView = {
        ...
        let barView = UIView()
        if UIDevice.current.userInterfaceIdiom == .pad {
            barView.pinConstraints(view, height: barViewheightForIpad, width: view.frame.width)
        }else {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            cancelButton.pinConstraints(barView, left: 0, top: 0, bottom: 0, width: buttonWidthForIpad)
            doneButton.pinConstraints(barView, right: 0, top: 0, bottom: 0, width: buttonWidthForIpad)
        }else {
        ...
        if let text = titleText {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                titleLabel.surroundConstraints(barView, left: buttonWidthForIpad, right: -buttonWidthForIpad)
            }else {
    ...
    private lazy var lineLabel: UILabel = {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            label.pinConstraints(view, height: lineHeightforIpad, width: view.frame.width)
        }else{
    ...
    private lazy var cancelButton: UIButton = {
        ...
        button.setTitleColor(buttonColor, for: .normal)
        if UIDevice.current.userInterfaceIdiom == .pad {
            button.titleLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: 32)
        }else {
            button.titleLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: 16)
        }
...
extension RPickerController: UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            pickerLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: 30)
        }else {
            pickerLabel?.font = UIFont(name: "HelveticaNeue-Bold", size: 16)
        }
